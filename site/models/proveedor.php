 ...
 * @since       1.5
 */
class ProveedorModelProveedor extends JModelForm
{
	/**
	 ...
	 * @var		string
	 */
	protected $_context = 'com_proveedor.proveedor';
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_proveedor')) &&  (!$user->authorise('core.edit', 'com_proveedor')))
		{
	...
	/**
	 * Method to get the proveedor form.
	...
	{
		// Get the form.
		$form = $this->loadForm('com_proveedor.proveedor', 'proveedor', array('control' => 'jform', 'load_data' => true));
		...
		$proveedor = $this->_item[$id];
		$params->merge($proveedor->params);

		if (!$params->get('show_email_copy', 0))
		{
			$form->removeField('proveedor_email_copy');
		}
	...
	{
		$data = (array) JFactory::getApplication()->getUserState('com_proveedor.proveedor.data', array());
	 ...
	 * Gets a proveedor
	 *
	 * @param   integer  $pk  Id for the proveedor
	 *
	...
	{
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('proveedor.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$data->tags->getItemTags('com_proveedor.proveedor', $data->id);
	 ...
	 *
	 * @return  mixed    The proveedor object on success, false on failure
	 *
	 * @throws  Exception  On database failure
	 */
	protected function getProveedorQuery($pk = null)
	{
		...
		if ($pk)
		{
			...
			if ($result)
			{
				...
				$proveedorParams = new Registry;
				$proveedorParams->loadString($result->params);
