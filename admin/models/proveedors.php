...
/**
 * Methods supporting a list of proveedor records.
 *
 * @since  1.6
 */
class ProveedorModelProveedors extends JModelList
{
	/**
	 * Constructor.
	 *
		...
		if ($assoc)
		{
			$query->select('COUNT(' . $db->quoteName('asso2.id') . ') > 1 as ' . $db->quoteName('association'))
				->join(
					...
					. ' AND ' . $db->quoteName('asso.context') . ' = ' . $db->quote('com_proveedor.item')
				)
				->join(
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_proveedor.proveedor')
				);
		}
